...
## Estrutura completa com Solution

```
projetoETL/
├── EtlProject.sln              # Solution — agrupa os dois projetos
...
└── EtlProject.Tests/           # Projeto de testes
    ...
    ├── TransformTests.fs        # Testes das funções puras
    └── Program.fs
```
...
```bash
dotnet build EtlProject.sln
cd EtlProject.Tests && dotnet run
```
